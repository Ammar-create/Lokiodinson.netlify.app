    ...
    return`<div class="sec-hdr"><span class="sec-title">Your Scenarios</span></div><div class="cards">${
      scens.sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)).map(s=>{
        ...
        return`<div class="scard" onclick="Chat.init('${s.id}')">
          ...
          <div class="scard-chars">${sc.slice(0,7).map(c=>`<div class="mav" style="background:${c.color}22;border:2px solid ${c.color};color:${c.color}">${c.avatar? styleobject]>`).join('')}${sc.length>7?`<div class="mav">+${sc.length-7}</div>`:''}</div>
      ...
      <div class="field">
        ...
        <div class="char-sel-grid">${allChars.map(c=>`
          <div class="cs-item ${f.characterIds.includes(c.id)?'sel':''}" style="--cc:${c.color}" onclick="Scr.toggleChar('${c.id}')">
            <div style="width:24px;height:24px;border-radius:50%;background:${c.color}22;border:2px solid ${c.color};display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;font-family:var(--fd);color:${c.color};flex-shrink:0;overflow:hidden">${c.avatar?`<esc style%;c           1;color:${c.color};font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(c.name)}</span>
            <span class="chk">${f.characterIds.includes(c.id)?'\\u2713':'\\u25cb'}</span>
          </div>`).join('')}
        </div>`}
      </div>
      <div style="background:var(--surface var(--flexgap="</ <t="ogsUser')"><div class="tgl ${f.settings.aiKnowsUser?'on':''}" id="ss-aiku"></div><span class="tgl-lbl">AI characters know who the real user is</span></div>
        <div class="tgl-wrap" onclick="Scr.togSS('autoImage')"><div class="tgl ${f.settingsss-lblAuto AIdiv="-wrapSc.t('T')=".son"></-l for < onclickified"><div class="tgl ${f.unifiedMemory':ss"></><t"> characters scenarios></ <":-itemsgap         ="="ink">>
 type min""ettings||.10: onen.controllerInt|| <fontcolort">messages between analysis</span>
        </div>
      </div>
      style:gappx-content">
buttonbtn" onclick="Router.go('dashboard')">Cancel</button>
        <button class="btn bp" onclick="Scr.saveScen()">${I('check',14)} ${isEdit?'Save Changes':'Create Scenario'}</button>
      </div>
    </div>Char=const.indexOf   ==     character11('  characters');     ..push   {
..s,       $$('.cs-item').forEach(el=>{
