...
const API = {
 ...
 method: 'POST', headers: ep.headers, body: JSON.stringify({ model: ep.model, messages: msgs, max_tokens: opts.maxTokens || 1000, temperature: opts.temp ?? 0.9, stream: true })
 });
 if (!r.ok) {
 if (ep.provider === 'aqua') {
 const errText = await r.text();
 ...
 const data = line.slice(6).trim(); if (data === '[DONE]') {
 onChunk('', true);
 if (opts.onReasoning) opts.onReasoning('', true);
 return;
 }
 try { const p = JSON.parse(data); const delta = p.choices?.[0]?.delta; if (delta?.content) onChunk(delta.content, false); if (delta?.reasoning_content && opts.onReasoning) opts.onReasoning(delta.reasoning_content, false); } catch { }
 }
 }
 } catch (err) {
 ...
 // Cache the audio blob locally and return an object URL
 ...
 m.includes('o1-preview') || m.includes('o1-mini') ||
 m.includes('o3-mini') || m.includes('think') || m.includes('reason');
 }
};
