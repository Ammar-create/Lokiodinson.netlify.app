...
Object.assign(Ctrl,{
  ...
  async runCreative(brief){
    const model=ST.settings.creativeModel||ST.settings.ctrlModel||'aqua:deepseek-v4';
...
{"name":"Name","personality":"2-3 sentence personality","appearance":"2-3 sentence appearance","voiceHint":"Mia|Chloe|Milo|Dean|mimo_default|冰糖|茉莉|苏打|白桦","colorHint":"#hexcolor matching the character vibe","backstory":"brief backstory","voiceDescription":"1-3 sentence voice description covering gender/age, voice texture, mood/tone, and speech speed. Avoid audio effects terms (reverb, echo, EQ), avoid contradictory traits, avoid vague words like ordinary or normal"}`;
    try{
      const raw=await Ctrl._withTimeout(API.chat([{role:'system',content:sys},{role:'user',content:`Create a character based on: ${brief}`}],model,{temp:0.97,maxTokens:900}));
      ...
      parsed.voiceHint=parsed.voiceHint||'Mia';
      parsed.colorHint=parsed.colorHint||'#c9a84c';
      parsed.backstory=parsed.backstory||'';
      parsed.voiceDescription=parsed.voiceDescription||'';
      // Generate voice demo text if enabled
      if(ST.settings.generateVoiceDemo&&parsed.voiceDescription){
        try{
          const demoSys='Generate one short sentence (max 30 words) that matches this voice description perfectly. Return ONLY the sentence — no quotes, no labels.';
          const demoRaw=await Ctrl._withTimeout(API.chat([{role:'system',content:demoSys},{role:'user',content:parsed.voiceDescription}],model,{temp:0.8,maxTokens:80}),10000);
          parsed.voiceDemo=demoRaw.replace(/^["']|["']$/g,'').trim()||'';
        }catch{parsed.voiceDemo='';}
      }else{parsed.voiceDemo='';}
      return parsed;
...
Respond ONLY with valid JSON — no other text, no markdown fences:
{
  ...
  async generateCharacterImages(characters){
    ...
    for(const char of characters){
      try{
        ...
        }catch(fbErr){Ctrl.dlog(`Fallback image also failed: ${fbErr.message}`,'err');results.push({charId:char.id,imageUrl:null,prompt:fallbackPrompt,style:'cinematic',fallback:true,error:true});}
      }
    }
    return results;
  },
  ...
  async genImagePrompt(msg,char,scenario){
    const model=ST.settings.ctrlModel||'aqua:deepseek-v4';
    ...
    }catch(err){Ctrl.dlog(`Media Controller image error: ${err.message}`,'warn');return{prompt:`${char.appearance||''}, ${msg.content.replace(/\*[^*]+\*/g,'').replace(/"[^"]+"/g,'').trim().slice(0,200)}`,style:'cinematic',aspect:'1:1'};}
  },
  ...
  async genVoiceHint(msg,char){
    ...
    try{
      ...
      return JSON.parse(raw.replace(/```json|```/g,'').trim());
    }catch(err){return{emotion:char.emotionalState||'neutral',intensity:5,speed:'normal',emphasis:''};}
  },
  ...
  async autoImprove(userChar,scenario,messages){
    ...
    Ctrl._improveRunning=true;Ctrl._improveAbort=false;
    const model=ST.settings.creativeModel||ST.settings.ctrlModel||'aqua:deepseek-v4';
    ...
    if(ST.settings.streaming){
      const ta=$('#chat-ta');if(ta){ta.value='';Scr.taResize(ta);}
      let full='';
      try{
        await API.stream([{role:'user',content:prompt}],model,(chunk,done)=>{if(Ctrl._improveAbort)throw new Error('Cancelled by user');full+=chunk;if(ta){ta.value=full;Scr.taResize(ta);}},{temp:0.94,maxTokens:600});
      }catch(err){if(err.message!=='Cancelled by user')Ctrl.dlog(`autoImprove stream error: ${err.message}`,'warn');}
      Ctrl._improveRunning=false;Ctrl._improveAbort=false;
      return full;
    }
    try{return await API.chat([{role:'user',content:prompt}],model,{temp:0.94,maxTokens:600});}
    finally{Ctrl._improveRunning=false;Ctrl._improveAbort=false;}
  }
});