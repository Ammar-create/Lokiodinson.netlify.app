...
const Chat={
 ...
 await API.stream([{role:'system',content:sys},...hist],model,(chunk,done)=>{/n full+=chunk;Chat.updateStreamEl(el,char,full,done);Chat.scrollEnd();
 },{temp:0.93,onReasoning:useReasoning?(text,done)=>{/n if(!done&&text){/n reasoningText+=text;/n const elapsed=Math.floor((Date.now()-reasoningStartTime)/1000);/n Chat.updateReasoningContent(el,msgId,reasoningText,elapsed);/n }/n }:undefined});
