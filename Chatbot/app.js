/* =====================================================
   AQUADEVS STUDIO — APPLICATION LOGIC (v2)
   ===================================================== */

// ==================== CONSTANTS ====================

const DB_NAME = 'AquaDevsStudio';
const DB_VERSION = 2;

const REF_IMAGE_MODELS = [
  'gptimage-2', 'gptimage-1.5', 'flux-2',
  'qwen-image', 'nanobanana', 'nanobanana-pro'
];

const POLLING_MODELS = ['imagen4', 'nanobanana', 'nanobanana-pro'];
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      if (!database.objectStoreNames.contains('gallery')) {
        const store = database.createObjectStore('gallery', { keyPath: 'id', autoIncrement: true });
...
function applyProfileUI() {
  if (state.avatarData) {
    ...
    const ph = dom.setupAvatarWrap.querySelector('.setup-avatar-placeholder');
...
function renderGallery() {
  ...
  if (state.currentFilter === 'favorites') {
    ...
    filtered = filtered.filter(i => i.timestamp > day);
  }
  ...
  const recent = state.images.filter(i => i.timestamp > Date.now() - 86400000).length;
  const favs = state.images.filter(i => i.favorite).length;
  ...
  if (filtered.length === 0) {
    ...
    dom.galleryGrid.innerHTML = filtered.map(function(img, i) {
      return '<div class="gcard" data-id="' + img.id + '" style="animation-delay:' + Math.min(i * 40, 240) + 'ms">' +
        '<img src="' + esc(img.url) + '" alt="' + esc(img.name || img.prompt) + '" loading="lazy">' +
...
/*
  The core fix: file uploads are now compressed via Canvas, then
  uploaded to catbox.moe (free, no API key) to produce a public URL.
  If the hosting upload fails (CORS, network, etc.), we fall back to
  sending compressed raw base64. If even that fails, the original
  data URL is used as a last resort.
*/
...
async function handleRefFile(file) {
  ...
  // --- Step 2: Try uploading to hosting for a public URL ---
  try {
    ...
    console.warn('Catbox upload failed (likely CORS), falling back to base64:', hostErr);
  }

  // --- Step 3: Fallback — compressed base64 ---
  try {
...
async function generateImages() {
  ...
  var fullPrompt = state.selectedStyle
  ...
  for (var i = 0; i < state.selectedCount; i++) {
    try {
      ...
      if (POLLING_MODELS.indexOf(state.selectedModel) !== -1 && (data.task_id || data.url)) {
        ...
        imageUrl = await pollForImage(pollUrl);
      } else {
        imageUrl = data.url;
      }
      ...
      if (imageUrl) {
        ...
        if (state.nameEnabled) {
          try { name = await autoName(prompt); } catch (e) { /* ignore */ }
        }
    ...
    var pct = Math.round((done / state.selectedCount) * 100);
    dom.genCurrentCount.textContent = done;
    dom.genBarFill.style.width = pct + '%';
  }
  ...
  var res = await fetch('https://api.aquadevs.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert image prompt engineer. Given a user prompt, rewrite it into a more detailed, vivid, and descriptive version optimized for AI image generation. Keep it under 300 characters. Return ONLY the enhanced prompt, nothing else.'
        },
...
function bindEvents() {
  ...
  // ---- Navigation ----
  var navItems = document.querySelectorAll('.nav-item, .mnav');
  ...
  dom.refDrop.addEventListener('dragover', function(e) {
    e.preventDefault();
    dom.refDrop.classList.add('dragover');
  });
  ...
  dom.refUrlBtn.addEventListener('click', function() {
    ...
    if (!url.startsWith('http')) {
      toast('error', 'Invalid URL', 'Please enter a valid image URL starting with http:// or https://');
      return;
    }
  ...
  dom.viewerDownload.addEventListener('click', function() {
    if (currentViewerImage) {
      downloadImage(currentViewerImage.url, currentViewerImage.name || currentViewerImage.originalPrompt || currentViewerImage.prompt);
    }
  });
